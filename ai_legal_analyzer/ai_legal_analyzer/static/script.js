...
async function analyze(){
  ...
  setLoading(true);
  try{
    const resp = await fetch('/api/analyze', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text})});
    const data = await resp.json();
    if(!resp.ok){outputEl.textContent = data.message||data.detail||'Error'}else{outputEl.textContent = data.analysis||data.insights||JSON.stringify(data,null,2)}
